...
public final class Util {
    public static void debug(Object... message) {
        ...
        Bukkit.broadcast(deserialize(format).color(NamedTextColor.BLUE)
                .append(deserialize(messages.toString()).color(NamedTextColor.GOLD)));
    ...
    public static void teleportEntity(LivingEntity e, Location loc, PlayerTeleportEvent.TeleportCause cause,
            boolean invulWhenTP) {
    ...
    public static boolean hasAbility(ItemStack item, @Nullable NamespacedKey key) {
        ...
        return item != null &&
                item.hasItemMeta() &&
                item.getItemMeta().getPersistentDataContainer().has(JKey.Ability) &&
                item.getItemMeta().getPersistentDataContainer().get(JKey.Ability, PersistentDataType.TAG_CONTAINER)
                        .has(key);
    }
     ...
     * @return {@link LivingEntity} target {@link LivingEntity} or null if not found
     *         or <b>not</b> a {@link LivingEntity}
     */
