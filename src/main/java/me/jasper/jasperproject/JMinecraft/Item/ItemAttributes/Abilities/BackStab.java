...
public class BackStab extends ItemAbility {
    ...
    public void action(BackStab e) {
        LivingEntity entity = Util.getTargetEntity(e.getPlayer(), e.getRange(), false);
        if (entity == null) {
        ...
        if (lokasiTujuan.toBlockLocation().getBlock().isSolid()
                || lokasiTujuan.clone().toBlockLocation().add(0, 1, 0).getBlock().isSolid()) {
            e.getPlayer().sendMessage(
                    MiniMessage.miniMessage().deserialize("<red><b>INVALID!</b> Location is obstructed"));
            return;
        }
        ...
        if (entity instanceof Player targetPlayer)
            targetPlayer.playSound(entity.getLocation(), Sound.ENTITY_GHAST_HURT, 1f, 0.85f);
        player.getWorld().spawnParticle(
                Particle.DUST, player.getLocation().add(0, 1, 0), 60, .3f, .4f, .3f, 0,
                new Particle.DustOptions(Color.fromRGB(60, 60, 60), 2f), false);
    }
    ...
    protected List<Component> createLore() {
        return List.of(
                MiniMessage.miniMessage()
                        .deserialize("<!i><gold>Ability: <red>BackStab <b><yellow>(SNEAK RIGHT CLICK)"),
                MiniMessage.miniMessage().deserialize("<!i><gray>Teleport to behind looked Entity in"),
                MiniMessage.miniMessage().deserialize(
                        "<!i><gray>range of " + this.range + " blocks and <color:#aa7a7f>BackStab</color>'em"));
    }
}
