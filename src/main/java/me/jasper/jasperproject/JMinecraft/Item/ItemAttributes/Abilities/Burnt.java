...
public class Burnt extends ItemAbility {
    ...
    public void action(Burnt e){
        ...
        e.getPlayer().getWorld().playSound(
                ...
                , Sound.ENTITY_BLAZE_HURT, SoundCategory.PLAYERS
                ,0.8f,1.56f
        );
        e.getPlayer().getWorld().playSound(
                e.getPlayer().getLocation()
                , Sound.ENTITY_FIREWORK_ROCKET_TWINKLE, SoundCategory.PLAYERS
                ,0.8f,1.85f
        );
        new BukkitRunnable() {
            private final Player p = e.getPlayer();
            private final Location clickLoc = e.getLocat();
            private final byte[] dir = direction[
                    Math.floorMod(Math.round(clickLoc.getYaw() / 90f), 4)
                    ];
            ...
            public void run() {
                ...
                if (this.frame > 0) this.clickLoc.getWorld().playSound(this.clickLoc.clone().add(dx, 0, dz)
                        , Sound.ENTITY_BLAZE_SHOOT, 0.5f, 1.68f);
    ...
    private boolean createFire(Location loc) {
        Location diAtas = loc.clone().add(0, 1, 0);
        Location diAtas2 = loc.clone().add(0, 2, 0);

        if (loc.getBlock().isSolid()) {
            if (diAtas.getBlock().isEmpty()) {
                diAtas.getBlock().setType(Material.FIRE);
                return diAtas.getBlock().getType() == Material.FIRE;
            }
            if (diAtas2.getBlock().isEmpty()) {
                diAtas2.getBlock().setType(Material.FIRE);
                return diAtas2.getBlock().getType() == Material.FIRE;
            }
        } else if (loc.clone().add(0, -1, 0).getBlock().isSolid() && diAtas.getBlock().isEmpty()) {
            loc.getBlock().setType(Material.FIRE);
            return loc.getBlock().getType() == Material.FIRE;

        } else if (diAtas.getBlock().isSolid() && diAtas2.getBlock().isEmpty()) {
            diAtas2.getBlock().setType(Material.FIRE);
            return diAtas2.getBlock().getType() == Material.FIRE;
        }
